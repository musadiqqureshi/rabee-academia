...
import { generateAssignmentWithAI } from "./actions";
...
export default function AIAssignmentForm({ batches }: { batches: BatchOption[] }) {
  ...
  function action(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        const id = await generateAssignmentWithAI(formData);
        router.push(`/dashboard/teacher/assignments/${id}`);
      } catch (e) {
  ...
  return (
    <div className="rounded-2xl border border-primary/30 bg-primary/5 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold inline-flex items-center gap-2"><Sparkles className="w-4 h-4 text-primary" /> Generate Assignment with AI</h3>
      ...
      <form action={action} className="space-y-3">
        <label className="block"><span className="block text-xs font-medium text-muted-foreground mb-1.5">Topic *</span>
          <input name="topic" required className={inputCls} placeholder="e.g. Balancing chemical equations" /></label>
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <label className="block"><span className="block text-xs font-medium text-muted-foreground mb-1.5">Total marks</span>
            <input type="number" name="total_marks" min={1} defaultValue={100} className={inputCls} /></label>
        </div>
        ...
        <button type="submit" disabled={pending}
          ...
          {pending ? <><Loader2 className="w-4 h-4 animate-spin" /> Generating…</> : <><Sparkles className="w-4 h-4" /> Generate Assignment</>}
        </button>
      </form>
    </div>
  );
