...
export default async function SuperAdminSchedules() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold">Platform Schedules</h1>
      <p className="text-sm text-muted-foreground mt-1">All class schedules across all batches</p>
      ...
      {sorted.length === 0 ? (
        <div className="mt-8 bg-card border border-card-border rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Schedules are created along with batches.</p>
        </div>
      ) : (
