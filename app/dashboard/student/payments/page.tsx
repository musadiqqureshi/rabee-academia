...
export default async function StudentPayments() {
  ...
  const { data: payments } = await supabase
    .from("payments")
    .select(`
      id, amount, currency, status, method, due_date, paid_at, notes,
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mt-1">Your payment history and status</p>
      ...
      {(!payments || payments.length === 0) ? (
        <div className="mt-8 bg-card border border-card-border rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Your payments will appear here after enrollment is verified.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/30">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Paid At</th>
