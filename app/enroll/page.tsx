...
import {
  ...
  Building2, Upload, Loader2, Clock, CalendarDays,
} from "lucide-react";
...
function EnrollContent() {
  ...
  useEffect(() => {
    const currentPath = `/enroll?subject=${slug}&type=${type}`;
  ...
  async function createEnrollment(paymentMethod: "assanpay" | "iban", receiptUrl?: string, assanpayRef?: string) {
    ...
    if (!subject) {
      setError("Subject not found. Please contact support.");
      return null;
    }
  ...
  async function handleAssanPay() {
    setLoading(true);
    setError(null);
    const id = await createEnrollment("assanpay", undefined, `ASSAN-${Date.now()}`);
    setLoading(false);
    if (id) {
      setEnrollmentId(id);
      setStep("success");
  ...
  async function handleIBANSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Continue even if upload fails — admin can ask for receipt separately
    }

    const id = await createEnrollment("iban", receiptUrl);
    setLoading(false);
    if (id) {
      setEnrollmentId(id);
      setStep("success");
        ...
        <AnimatePresence mode="wait">
          ...
          {step === "details" && (
            <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              ...
              <h1 className="text-2xl font-extrabold mb-2">Your Details</h1>
              <p className="text-sm text-muted-foreground mb-6">Pre-filled from your account. Update if needed.</p>
              ...
              {payMethod === "assanpay" && (
                <div className="bg-card border border-border rounded-2xl p-6">
                  ...
                  <Button onClick={handleAssanPay} disabled={loading} className="w-full font-bold">
                    ...
                    {loading ? "Submitting…" : `Pay ${formatPrice(price)} via AssanPay`}
                  </Button>
                </div>
              )}
            ...
            <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.25 }} className="text-center py-10">
              ...
              {enrollmentId && (
                <p className="text-xs text-muted-foreground mb-6">
                  Reference ID: <span className="font-mono text-foreground">{enrollmentId}</span>
                </p>
              )}
